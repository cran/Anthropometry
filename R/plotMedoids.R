plotMedoids <- function(x,medoids,nsizes,bustVariable,variable,color,xlim,ylim,title,EN){
 ...
 if(variable == "chest"){
  ...
  for(i in 1 : (nsizes-1)){
  #To locate correctly the rows of the medoids in the whole database, the medoids labels must be 
  #converted into a character.
   points(x[as.character(medoids[[i]]), bustVariable], x[as.character(medoids[[i]]), variable], pch = i, 
          col = color[i])
  }
  ...
  if(EN){
   ...
   #(see Ibañez et al. (2012)), to the nearest integer:
