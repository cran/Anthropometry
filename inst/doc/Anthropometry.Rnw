...
As also noted above, a 3D binary image of each woman's trunk is available. Hence, the dissimilarity between trunk forms can be computed and a distance matrix between women can be built. The distance matrix used in \cite{Vinue2013Arch} is included in \pkg{Anthropometry} and is called \code{descrDissTrunks}. 
...
As a conclusion to this discussion, an illustrative comparison of the outcomes of using \emph{trimowa} and \emph{TDDclust} on a random sample subset is given below. We restrict our attention to these two methods because both of them have the same intention. The \emph{trimowa} and \emph{TDDclust} methods are implemented in the \code{trimowa} and \code{TDDclust} functions, respectively. More details about the use of these functions are given in Section \ref{examples}. We run both algorithms for twenty randomly selected women. To reproduce the results, a seed for randomness is fixed. The arguments that share both algorithms are given the same value. They are \code{numClust} (number of clusters), \code{alpha} (trimmed percentage), \code{niter} (number of iterations) and \code{verbose} (to provide descriptive output on progress). 
...
For practical guidance, the method used for clustering-based approaches is as follows: Firstly, the data matrix is segmented using a primary control dimension (bust circumference in the case of \emph{trimowa}, \emph{hipamAnthropom}, \emph{kmeansProcrustes} and \emph{TDDclust}, and waist circumference in the case of \emph{CCbiclustAnthropo}, according to the classes suggested in the European standard on sizing systems. Size designation of clothes. Part 3: Measurements and intervals \citep{NormaUNE3}. This standard is drawn up by the European Union and is a set of guidelines for the textile industry to promote the implementation of a clothing sizing system, that is adapted to users). Then, a further clustering segmentation is carried out using other secondary control anthropometric variables. In this way, the first segmentation provides a first easy input to choose the size, while the resulting clusters (subgroups) for each bust (or waist) and other anthropometric measurements optimize the sizing. From the point of view of clothing design, by using a more appropriate statistical strategy, such as clustering, homogeneous subgroups are generated taking into account the anthropometric variability of the secondary dimensions that have a significant influence on garment fit.
...
\STATE 9. If no moves were accepted for the last $M$ iterations and $\beta < \infty$, set $\beta=\infty$ and iterate 2-8. If no moves were accepted for the last $M$ iterations and $\beta=\infty$. Otherwise, set $\beta=2\beta$ and iterate 2-8.
}
\end{algorithmic}
\end{algorithm}
...
\STATE The candidate partition, $C=\{C_1, \ldots, C_k\}$, from $P$ is obtained.
\ENDIF
\ENDIF
}
\end{algorithmic}
...
\ELSE \STATE C is rejected. STOP (P is a terminal node).
\ENDIF
}
\end{algorithmic}
\end{algorithm} 
...
\STATE 3. Steps 1. and 2. are repeated until convergence of the algorithm.
}
\end{algorithmic}
\end{algorithm}
...
%Hartigan (mirar el paper de Amaral)
\begin{algorithm} \label{Hartigan}
...
\STATE 7. If no transfer took place in the last $n$ steps, go to Step 4. Otherwise, go to Step 6.
}
\end{algorithmic}
\end{algorithm}
...
Next the \code{trimowa} algorithm is used within each bust class. Three size groups (clusters, argument \code{numClust}) are calculated per bust segment. This number of groups is quite well aligned with the strategy used by companies to design sizes. A larger \code{numClust} will result in many sizes being designed, increasing the production a lot. A smaller \code{numClust} corresponds to too few sizes being designed and having a poor accommodation index. 
...
The prototypes are the clustering medoids. The \code{anthrCases} function allows us to obtain the estimated cases by each method.
...
Figure~\ref{trimowaExample} shows the scatter plots of bust circumference against neck to ground with the three prototypes obtained for each bust class without (left) and with (right) the prototypes defined by the European standard. The prototypes color and the plot title must be provided.
...
The fit models are the clustering medoids. The \code{trimmOutl} function allows us to obtain the trimmed or outlier observations identified by each method.
...
The following sentences allow us to analyze the clustering results, the final value of the optimal partition and the iteration in which the optimal partition was found, respectively.
...
The prototypes and trimmed observations are obtained with the functions created to that end:
...
In this section, the use of the \emph{kmeansProcrustes} methodology is illustrated. For the sake of simplicity of the computation involved only a small sample (the first 50 individuals) is selected. When there are missing values (na's, not available numbers), they are removed.
...
The clustering results and the optimal prototypes are obtained in the following way:
...
Figure~\ref{AAExample} shows the percentils of three archetypoids, beginning from \emph{nearest} (left) and with \emph{which} (right).
...
\centering
\begin{tabular}{cc}
\hspace*{-2.6cm}\includegraphics[width=0.4\textwidth]{prendas_1.png} &
\hspace*{-3cm}\includegraphics[width=0.4\textwidth]{prendas_2.png} \\
\hspace*{3cm}\includegraphics[width=0.5\textwidth]{prendas_3.png} \\
\end{tabular}
\vspace*{-1.2cm}
