...
initialize.tree <- function(x, maxsplit, orness, type, ...){
 if(type == "MO"){
  x.ps <- getBestPamsamMO(x, maxsplit, orness, type, ...)
 }else{
   x.ps <- getBestPamsamIMO(x, maxsplit, orness, type, ...)
  }
...
ext.dist <- function(x, maxsplit, orness, ahVect=c(23, 28, 20, 25, 25)){
 ...
 DIST <- GetDistMatrix(datost, num.personas, num.variables, w, bl, bh, al, ah, T) #T means TRUE.
...
pamsam <- function(x, k = k, type = type, DIST, maxclust = 50, further.maxclust = 20, exhaustive = FALSE, 
 ...
 if(type == "MO"){                 
  #Calculate DIST:
  DIST <- ext.dist(x, maxsplit = k, orness = orness, ahVect=ahVect)
 }else{
   DIST <- DIST
  }
...
further.clustering <- function(x, k, DIST, medoids, clustering, asw, sesw, max.check, total.check, maxclust, 
                               exhaustive, maxsplit, orness, ahVect){
...
collapse.cluster <- function(x, k, DIST, j, medoids, clustering, asw, sesw, maxclust, exhaustive, maxsplit, 
                             orness, ahVect=c(23, 28, 20, 25, 25)){

 if(k > 2){ 
  DIST.medoids <- ext.dist(medoids, maxsplit, orness = orness, ahVect=ahVect) 
...
hipam.local <- function(tree, x, asw.tol, maxsplit, local.const, orness, type, ...){
 ...
 while(sum(active) > 0){
  choose.among <- (1:length(active))[active]
   ...
   if(type == "MO"){
    proposal <- checkBranchLocalMO(tree, x, whch.brnch, maxsplit, asw.tol, local.const, orness, type, ...)
   }else{
     proposal <- checkBranchLocalIMO(tree, x, whch.brnch, maxsplit, asw.tol, local.const, orness, type, ...)
    }
